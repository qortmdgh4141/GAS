...
from M_utils.networks import MLP
...
class ResnetStack(nn.Module):
    ...
    def __call__(self, x):
        ...
        conv_out = nn.Conv(features=self.num_features, kernel_size=(3, 3), strides=1, kernel_init=initializer, padding='SAME',)(x)
        if self.max_pooling:
...
class ImpalaEncoder(nn.Module):
    ...
    def setup(self):
        ...
        self.stack_blocks = [ResnetStack(num_features=stack_sizes[i] * self.width, num_blocks=self.num_blocks,) for i in range(len(stack_sizes))]
...
class GCEncoder(nn.Module):
    ...
    It takes in observations (s) and goals (g) and returns the concatenation of `state_encoder(s)`, `goal_encoder(g)`, and `concat_encoder([s, g])`. 
    It ignores the encoders that are not provided. 
    This way, the module can handle both early and late fusion (or their variants) of state and goal information.
    """
    ...
    def __call__(self, observations, goals=None, goal_encoded=False):
        ...
        If `goal_encoded` is True, `goals` is assumed to be already encoded representations. 
        In this case, either `goal_encoder` and `concat_encoder` must be None.
        """
        reps = []
