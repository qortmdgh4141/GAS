    ...
    def __call__(self, *args, name=None, **kwargs):
        ...
        if name is None:
            if kwargs.keys() != self.modules.keys():
                raise ValueError(f'When `name` is not specified, kwargs must contain the arguments for each module. ' f'Got kwargs keys {kwargs.keys()} but module keys {self.modules.keys()}')
            out = {}
...
class TrainState(flax.struct.PyTreeNode):
    ...
    def apply_loss_fn(self, loss_fn):
        ...
        info.update({'grad/max': final_grad_max, 'grad/min': final_grad_min, 'grad/norm': final_grad_norm,})
